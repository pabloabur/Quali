...
CRs disparando espontaneamente, com exceção da seção \ref{sec:met_onion}.
...
Nas simulações com inibição recorrente, $mvcPerc=310$ foi suficiente
...
situação de fuga de uma ameaça repentina. Outros cenários analisados
...
\citeonline{walmsley78}. Essas situações
foram analisadas por meio do coeficiente de variação da força,
calculado pela equação (\ref{eq:cv}) mostrada abaixo:
...
(metade da condutância) e forte (dobro da condutância).
Nesses dois últimos casos, o valor dessas frequências de disparos foi
ajustado para cada caso, como na seção \ref{sec:met_onion}.
