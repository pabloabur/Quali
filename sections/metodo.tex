...
Para possibilitar que o problema abordado pudesse ser unidades menores de 
computação, foram identificados algumas \textit{tasks} primitivas: os cálculos
...
Serão utilizados 2, 4, 8 e 16 \textit{cores} de um único servidor. As unidades
motoras serão divididas igualmente entre os processadores disponíveis, 
enquanto que a \textit{task} de cálculo da força deve ocupar sozinha um único
processador.

Os parâmetros que serão usados para analisar a qualidade do programa paralelo são
o \textit{speed up} (ou aceleração), calculado pela equação
...
Uma força com 20\% da contração máxima voluntária será gerada e dela foram
