...
Poisson e com conectividade de 30\%.
...
\begin{table}[ht!]
\caption{Dados experimentais utilizados na parametrização dos MNs.}
\label{tab:met_MNparams}
\centering
    \begin{tabular}{p{3.5cm} c c c p{3.5cm}}\hline 
        \multirow{2}{*}{Propriedades} & \multicolumn{3}{c}{Unidades motoras} &
            \multirow{2}{*}{Referência} \\
        & FF & FR & S & \\ \hline 
        Intervalo da tensão tetânica máxima (g) & 30 até 130 &
            4.5 até 55 & 1.2 até 12.6 & \citeonline{burke71} \\
        Mediana da tensão tetânica máxima (g) & 60 & 20 & 5 & 
        \citeonline{burke71} \\
        Intervalos de tempo de contração do \textit{twitch} (ms) & 20 até 47 &
            30 até 55 & 58 até 110 & \citeonline{burke71} \\
        Tensão do \textit{twitch} (g) & 6.7$\pm$ 0.8 & 0.3$\pm$ 0.1 & 0.4$\pm$ 0.1 &
            \citeonline{fleshman81} \\
...
com as CRs (condutâncias sinápticas e conectividade) sejam as mesmas.
...
de força reportados na Figura \ref{fig:met_granit}. Além disso,
a atividade espontânea das CRs foi ativada.
...
Para reproduzir os resultados encontrados por \citeonline{williams09}, um
cenário de simulação semelhante ao desses autores foi emulado. Com o
intuito de fazer com que os MNs disparem a uma taxa aproximada de 10 Hz,
a frequência de disparos das fibras descendentes que os ativavam
foi modelada como uma onda quadrada com período de 100 ms. Essa possui
picos com duração de 10 ms e amplitude de 15 Hz, enquanto que seus vales
são nulos.

Ademais, cada MN também foi ativado por um ruído excitatório
com o mesmo formato, mas com condutância sináptica máxima de 12 $\mu$S.
Essa ativação, além de introduzir mais variabilidade nos disparos,
foi utilizada para compensar a redução da força causada pela
inibição recorrente. Nesse caso, as amplitudes dos picos foram de 3300 e
1200 Hz em simulações com e sem CR, respectivamente. Nota-se,
ainda, que
os disparos dessa a entrada excitatória ocorre de acordo com
um processo pontual Poisson e que, sem as CRs, a média de condutâncias
...
da população de MNs \cite{hoffer87,walmsley78}. Sendo assim, 
a simulação foi efetuada com apenas 75 MNs do tipo S distribuídos em 1.5 mm de 
medula espinhal. Vale notar que, como \citeonline{williams09} consideraram
...
do tipo FR, se estivessem presentes. Por fim, a população de CRs foi a mesma
...
suficiente para gerar valores de força próximos ao MVC. Essa taxa
foi mantida por 500 ms.
...
situação de fuga de uma ameaça repentina. Outro cenário analisado
foi o da manutenção da posição vertical com 5\% do MVC. Essa situação
foi simulada por meio de fibras descendentes com taxas constantes e foi
analisada por meio do coeficiente de variação e desvio padrão da força.
Por fim, o coeficiente de sincronia entre os MNs também foi calculado.

Com o intuito de entender o efeito de diferentes forças de inibição
recorrente nos cenários propostos, a condutância sináptica das CRs agindo
sob os MNs, além de ser mantida em seus valores normais,
foi alterada para contemplar situações de inibição fraca
(metada da condutância) e forte (dobro da condutância).
