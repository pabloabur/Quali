...
Nas simulações com inibição recorrente, $cvmPerc=950$ foi suficiente
...
As entradas sinápticas utilizadas nas simulações descritas aqui
foram: Fibras descendentes com taxas de disparos moduladas por 3 senoides e 
um determinado valor médio agindo sob o núcleo motor e ruídos independentes
...
ruído, por sua vez, tiveram condutância sináptica máxima de 12 $\mu$S e 
...
que fossem geradas forças dentro de uma faixa de 10 \% em torno da força
gerada na situação anterior. Dessa forma, os valores utilizados foram de
300 e 438 Hz.

A duração das simulações foi de 10 s, sendo que os primeiros 2 s dos
sinais gerados foram retirados das análises subsequentes para eliminar 
possíveis respostas transitórias. A representação no domínio da frequência
da força muscular resultante foi obtida por meio da transformada de Fourier. 
Denominando \Bi como o pico do módulo da resposta em frequência em
...
\begin{equation}
    \frac{\sqrt{B_1^2+B_2^2+B_3^2}}{\sqrt{\sum B_j^2}}
    \label{eq:pbipbni}
\end{equation}
