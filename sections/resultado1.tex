...
\ref{fig:RCepsp} foi obtido com $R_m=\text{8500 }\Omega\text{cm}^2$
$C_m = \text{20 }\mu\text{Fcm}^{-2}$. O PEPS é
iniciado em 10.35 ms e atinge o pico em 16.15 ms, configurando, assim, um
tempo de subida de 5.80 ms. A duração total observada foi de aproximadamente
50 ms.
...
na duração do PEPS. Vale notar que a constante de tempo $\tau$ foi de 8 ms,
que é um valor condizente com trabalhos experimentais e de simulação 
\cite{hultborn79,maltenfort98}. Portanto, a parametrização realizada foi tomada
...
a Equação (\ref{eq:vth}) resulta em $V_l=\text{1.14804 }$ mV.
...
resultante foi de 45.92 M$\Omega$. Esse valor é condizente com o fato
...
dos implementados aqui pode explicar essa diferença. Sabe-se que 
