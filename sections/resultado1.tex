...
simulações com diferentes números de processos. Como se pode perceber, foi 
...
bem próximos para 2, 4 e 8 processos, mas uma queda começa a se tornar visível com
16 processos, sugerindo que, a partir desse número, a queda de eficiência
se acentue.
...
\begin{figure}[ht]
    ...
    \caption[Resultados de aceleração e eficiência em função do número de processos]{
        Resultados de aceleração e eficiência em função do número de processos.
        \subref{fig:clustera} Os círculo vazio representam o \textit{Speedup} obtido para cada
        número de processos utilizado. A reta em azul mostra uma regressão linear dos
        resultados. \subref{fig:clusterb} eficiência em função do número de processos.
         }
\end{figure}

Outro resultado sobre desempenho computacional obtido foi o de tempo de
execução com as matrizes de condutância alteradas no código em Python. Como mostrado na 
Tabela \ref{tab:matrixG}, não foi observada uma melhora de desempenho quando se 
usa a \Gn com 10 MNs. Entretanto, à medida em que se aumenta
...
26\% em relação à $G_a$.
Isso sugere que as otimizações trazidas
por essa estratégia só são vantajosas para simulações que envolvam \Gn
relativamente grandes.

\begin{table}[ht]
\caption{Tempo de execução das simulações
         com diferentes matrizes de condutâncias e quantidades de MNs.}
\label{tab:matrixG}
\centering
...
Os resultados mostrados anteriormente são positivos, mas possuem 
limitações. Em primeiro lugar, é preciso enfatizar que os MNs das simulações
...
(são os chamados problemas embaraçosamente paralelos). 
...
sinápticas, seriam necessárias muitas comunicações entre processos
...
no trabalho de \citeonline{morrison05}. Entretanto, se essa estratégia
for adotada no ReMoto, seria aconselhável utilizar a versão em
Fortran por causa de seu desempenho superior.

Vale notar que, como o Fortran possui suporte nativo à operações 
vetorizadas, a implementação de $G_n$ já foi realizada por R. N. 
Watanabe (informação pessoal) na versão do ReMoto em Fortran.
...
\begin{figure}[ht!]
    ...
    \caption[Potencial excitatório pós-sináptico em uma célula de Renshaw]{
...
sobre conectividade e foram abordadas na seção \ref{sec:conn}. A utilização
de outros valores de resistência para diminuir a amplitude de PEPS, como
feito por \citeonline{williams09}, alteraram
significativamente as características de disparos mostrados na
seção \ref{sec:res_firing} e, por isso, não foram adotados.
...
na duração do PEPS. Portanto, a parametrização realizada satisfez
razoavelmente cada característica.
...
haja uma superposição nos valores da variável de ativação \qt,
fazendo com que exista uma corrente iônica
abaixando o potencial de membrana para o equilíbrio do canal de potássio
lento (-10 mV). Esse comportamento é mostrado na Figura \ref{fig:qt}. 
...
\begin{figure}[ht!]
    ...
    \caption[Evolução temporal da variável de transição $q(t)$]{
        Evolução temporal da variável de ativação $q(t)$.}
	\label{fig:qt}
\end{figure}
...
\begin{figure}[ht]
    ...
    \hspace{-1cm}
    \caption[Característica de disparos da célula de Renshaw]{
        Característica de disparos da célula de Renshaw.
...
\begin{figure}[ht!]
    ...
    \caption[Disparos espontâneos de uma célula de Renshaw]{
        Disparos espontâneos de uma célula de Renshaw.}
	\label{fig:spont}
\end{figure}
...
mas os três primeiros intervalos foram mais rápidos do que o esperado,
...
\begin{figure}[ht]
    ...
    \caption[Comparação dos potenciais de membrana para uma corrente injetada
        ...
        CR tem uma duração de aproximadamente 30 ms. Fonte: Adaptado de \citeonline{hultborn79}.
        \subref{fig:resmemb} Resultado obtido pelo modelo de uma célula de Renshaw nas
        mesmas condições.
         }
    \label{fig:FxIcomp}
\end{figure}
...
\begin{enumerate}[label=\alph*)]
    ...
    \item como as conexões sinápticas relacionadas com as CRs são diferentes
          ...
          e os dados em questão foram obtidos em um estágio pós-natal
          precoce, não se pode garantir que estas propriedades sejam
...
\begin{figure}[ht!]
    ...
    \caption[Potencial inibitório pós-sináptico de uma célula de Renshaw]{
...
O
padrão de decaimento com a distância foi semelhante ao que foi apresentado
...
experimentais na ausência da dinâmica das sinapses foi, possivelmente,
a mesma apresentada por meio da Figura \ref{fig:FxIcomp}. Um 
aumento da condutância sináptica dos MNs sobre as CRs traria resultados
...
Universidade de Düsseldorf, 1990.}
...
Em suma, os resultados das parametrizações realizadas nessa seção
foram sumarizados na Tabela \ref{tab:params_final}.
No ReMoto, os valores desses parâmetros devem ser fornecidos ao programa
por meio de um arquivo de configuração.

\begin{table}[ht!]
\caption{Valores a serem atribuídos aos parâmetros das células de Renshaw no arquivo de
    configuração do ReMoto.}
\label{tab:params_final}
\centering
    \begin{tabu}{c C{3cm} C{3cm}}\tabucline[1.5pt]{-} 
	Parâmetro & Valor & Unidade \\ \tabucline[1.5pt]{-} 
